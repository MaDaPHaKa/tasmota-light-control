...
export class SnackbarService {
  ...
  error(message: string): void { this.open(message, 'error'); }
  ...
  private open(message: string, panelClass: string): void { this.snackBar.open(message, 'Close', { duration: 10_000, panelClass: [`snackbar-${panelClass}`], horizontalPosition: 'right', verticalPosition: 'bottom' }); }
}
